package com.example.svenfulenchek_wguscheduler.ui;

// This class stores constant values and utility functions
public class utils {
    public static final int DATABASE_VERSION = 17;

    // Request codes for all activities are handled HERE
    public static final int COURSE_VIEW_RETURN = 7;
    public static final int TERM_LIST_RETURN = 9;
    public static final int TERM_VIEW_RETURN = 8;

    public static final int ADD_TERM_REQUEST_CODE = 10;
    public static final int EDIT_TERM_REQUEST_CODE = 11;
    public static final int DELETE_TERM_REQUEST_CODE = 12;

    public static final int ADD_COURSE_REQUEST_CODE = 20;
    public static final int EDIT_COURSE_REQUEST_CODE = 21;
    public static final int DELETE_COURSE_REQUEST_CODE = 22;

    public static final int ADD_ASSESSMENT_REQUEST_CODE = 30;
    public static final int EDIT_ASSESSMENT_REQUEST_CODE = 31;
    public static final int DELETE_ASSESSMENT_REQUEST_CODE = 32;


}
